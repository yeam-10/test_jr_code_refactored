...
class ProjectProject(models.Model):
    ...
    def create_invoice_line(self, invoice):
        ...
        def create_invoice_line_for_employee(employee_data, temp_time, invoice, sale_order_obj):
            product_id = employee_data['inhouse'] if employee_data['company_id'] == self.env.user.company_id.id else employee_data['outsource']
            if sale_order_obj.pricelist_id and sale_order_obj.partner_id:
                product = self.env['product.product'].search([('id', '=', employee_data['product_id'])])
                vals = self.env['account.tax']._fix_tax_included_price(product.price, product.taxes_id, sale_order_obj.tax_id)
            else:
            ...
            self.env['account.invoice.line'].create({
                ...
                'name': employee_data['product_name'] + ': ' + employee_data['employee'],
                'quantity': temp_time,
                'price_unit': vals,
                ...
                if array:
                    for euser in array:
                        ...
                        if not emp_obj:
                            ...
                            create_invoice_line_for_employee(employee_data, temp_time, invoice, sale_order_obj)
                            ...
                            create_invoice_line_for_employee(emp_obj, temp_time, invoice, sale_order_obj)
